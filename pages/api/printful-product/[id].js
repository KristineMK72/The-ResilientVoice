...
export default function ProductPage({ initialProduct, productId }) {
  ...
  /* -----------------------------
     ...
     Priority:
     - admin gallery images from DB
     - local /{id}_1.png, /{id}_2.png, etc
     - selected variant image_url
     - selected variant preview_url
     - product thumbnail
  ------------------------------ */
  const adminGalleryCandidates = useMemo(() => {
  ...
  useEffect(() => {
    ...
    async function probeImages() {
      const candidates = [...adminGalleryCandidates, ...localCandidates, ...remoteCandidates].filter(Boolean);
  ...
  }, [adminGalleryCandidates, localCandidates, remoteCandidates]);
